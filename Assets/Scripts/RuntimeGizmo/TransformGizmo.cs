...
namespace RuntimeGizmos
{
    ...
    public class TransformGizmo : MonoBehaviour
    {
        ...
        void Awake()
        {
            ...
            SetMaterial();
            selectionMask = LayerMask.GetMask("hidden_surface", "selected_surface");
        ...
        void GetTarget()
        {
            if (nearAxis == Axis.None && isClick())
            {
                ...
                RaycastHit hitInfo;
                if (Physics.Raycast(mouseRay, out hitInfo, Mathf.Infinity, selectionMask))
                {
        ...
        private List<TransformType> TypeList = new List<TransformType>() { TransformType.Move, TransformType.Rotate, TransformType.Scale, TransformType.All };

        private int currTypeIndex = 0;

        public void nextType()
        {
            currTypeIndex = TypeList.IndexOf(transformType);
            int nextIndex = currTypeIndex + 1 >= TypeList.Count ? 0 : currTypeIndex + 1;
            transformType = TypeList[nextIndex];
        }

        public void lastType()
        {
            currTypeIndex = TypeList.IndexOf(transformType);
            int lastIndex = currTypeIndex - 1 < 0 ? TypeList.Count - 1 : currTypeIndex - 1;
            transformType = TypeList[lastIndex];
        }
